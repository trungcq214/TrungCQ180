...
<html>
    ...
    <body>
        ...
        <div class="content">
            <div class="header">
                <h1>
                    <a href="adminRooms?theaterId=${requestScope.theater.theaterId}" class="btn-back" title="Back to Rooms">&larr;</a>
                    Seats in ${requestScope.room.name} (${requestScope.theater.name})
                </h1>
            ...
            <button class="btn btn-add" onclick="openAddModal()">+ Add New Seat</button>
            
            <table>
                <thead>
                    <tr>
                        <th>Seat ID</th>
                        <th>Seat Name (e.g., A1)</th>
                        <th>Type</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <c:forEach items="${requestScope.seats}" var="s">
                        <tr>
                            <td>${s.seatId}</td>
                            <td><strong>${s.seatName}</strong></td>
                            <td>${s.type}</td>
                            <td>
                                <button class="btn btn-edit" onclick="openEditModal(${s.seatId}, '${s.seatName.replace('\'', '\\\'')}', '${s.type}')">Edit</button>
                                
                                <form action="adminSeats" method="POST" style="display:inline;" onsubmit="return confirm('Delete this seat? Note: Fails if there are booked tickets mapped to this seat!');">
                    ...
                    <c:if test="${empty requestScope.seats}">
                        <tr><td colspan="4" style="text-align:center;">No seats found. Wait... they should auto-generate!</td></tr>
                    </c:if>
                </tbody>
            </table>
        </div>
        ...
        <div id="seatModal" class="modal">
            <div class="modal-content">
                ...
                <h2 id="modalTitle">Add Seat</h2>
                <form action="adminSeats" method="POST">
                    <input type="hidden" name="action" id="formAction" value="add">
                    <input type="hidden" name="theaterId" value="${requestScope.theater.theaterId}">
                    <input type="hidden" name="roomId" value="${requestScope.room.roomId}">
                    <input type="hidden" name="seatId" id="seatId" value="0">
                    
                    <div class="form-group">
                        <label>Seat Name</label>
                        <input type="text" name="name" id="name" required placeholder="e.g. D1">
                    </div>
                    <div class="form-group">
                        <label>Type</label>
                    ...
                    <button type="submit" class="btn btn-add" style="width: 100%;">Save Seat</button>
                </form>
            </div>
        </div>
        ...
        <script>
            ...
            function openAddModal() {
                document.getElementById("modalTitle").innerText = "Add Seat";
            ...
            function openEditModal(id, name, type) {
                document.getElementById("modalTitle").innerText = "Edit Seat";
