...
<html>
    ...
    <body>
        ...
        <div class="content">
            ...
            <button class="btn btn-add" onclick="openAddModal()">+ Add New Theater</button>
                ...
                <tbody>
                    <c:forEach items="${requestScope.theaters}" var="t">
                        <tr>
                            ...
                            <td>${t.address}</td>
                            <td>
                                <span style="color: ${t.isActive ? 'green' : 'red'}; font-weight: bold;">
                                    ${t.isActive ? 'Active' : 'Inactive'}
                                </span>
                            </td>
                            <td>
                                <a href="adminRooms?theaterId=${t.theaterId}" class="btn btn-rooms">Manage Rooms</a>
                                <button class="btn btn-edit" onclick="openEditModal(${t.theaterId}, '${t.name.replace('\'', '\\\'')}', '${t.address.replace('\'', '\\\'')}')">Edit</button>
                                
                                <c:if test="${t.isActive}">
                                    <form action="adminTheaters" method="POST" style="display:inline;" onsubmit="return confirm('Disable this theater?');">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="theaterId" value="${t.theaterId}">
                                        <button type="submit" class="btn btn-delete">Disable</button>
                                    </form>
                                </c:if>
                            </td>
                        </tr>
        ...
        <div id="theaterModal" class="modal">
            <div class="modal-content">
                ...
                <h2 id="modalTitle">Add Theater</h2>
                <form action="adminTheaters" method="POST">
                    <input type="hidden" name="action" id="formAction" value="add">
                    <input type="hidden" name="theaterId" id="theaterId" value="0">
                    ...
                    <button type="submit" class="btn btn-add" style="width: 100%;">Save Theater</button>
                </form>
            </div>
        </div>

        <script>
            var modal = document.getElementById("theaterModal");

            function openAddModal() {
                document.getElementById("modalTitle").innerText = "Add Theater";
                document.getElementById("formAction").value = "add";
                document.getElementById("theaterId").value = "0";
            ...
            function openEditModal(id, name, address) {
                document.getElementById("modalTitle").innerText = "Edit Theater";
                document.getElementById("formAction").value = "edit";
                document.getElementById("theaterId").value = id;
